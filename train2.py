...
def trainer(sweep_config = None):
    from config import config
    increment()
    config['prefix'] = 'test1_' + str(COUNT) + '_' + today_timeinfo() 
    print(config['prefix'])
    ...
    with wandb.init(config = sweep_config):
        ...
        ##### 하이퍼 파라미터 추가 또는 삭제하는 부분 ######
        # 완디비 sweep config 에 맞게 수정하면 된다!

        config['batch_size'] = config_wandb.batch_size
        config['lr'] = config_wandb.learning_rate
        config['focal_gamma'] = config_wandb.focal_gamma
        config['shared_dim1'] = config_wandb.shared_dim1
        config['shared_dim2'] = config_wandb.shared_dim2
        config['task_overall'] = config_wandb.task_overall
        ...
        tr_loader, val_loader, _ = train_val_test_loader(config)
        ...
        for epoch_i in range(0, config['epochs']):
            ...
            wandb.log({"total_validation_loss": total_validation_loss, "epoch": epoch_i})
            ...
            if (config['save_mode'] == True) and (epoch_i > 3):
